 ...
 * @author Carl
 */
public class BlockTerrainControl extends AbstractControl implements BitSerializable, BTControl{
        ...
        for(int x=0;x<chunks.length;x++){
            for(int y=0;y<chunks[0].length;y++){
                for(int z=0;z<chunks[0][0].length;z++){
                    BlockChunkControl chunk = new BlockChunkControl(this, new Vector3Int(x, y, z));
                    chunks[x][y][z] = chunk;
                }
            }
        }
    ...
    private Vector3Int getChunkLocation(Vector3Int blockLocation){
        ...
        int chunkX = (blockLocation.getX() / GameProperties.CHUNK_SIZE.x);
        int chunkY = (blockLocation.getY() / GameProperties.CHUNK_SIZE.y);
        int chunkZ = (blockLocation.getZ() / GameProperties.CHUNK_SIZE.z);
        chunkLocation.set(chunkX, chunkY, chunkZ);
        return chunkLocation;
    }

    /**
     * Experimental. Should convert a Vector3f location to a block location
     */
    public Vector3Int getBlockLocation(Vector3f loc)
    {
        loc = Util.compensateFloatRoundingErrors(loc);
        int x = (int) Math.floor((float) loc.x / GameProperties.CUBE_SIZE);
        int y = (int) Math.floor((float) loc.y / GameProperties.CUBE_SIZE);
        int z = (int) Math.floor((float) loc.z / GameProperties.CUBE_SIZE);
        return new Vector3Int(x, y, z);
    }

    public Vector3f getBlockTranslation(Vector3Int loc)
    {
        return new Vector3f(loc.getX()*GameProperties.CUBE_SIZE, loc.getY()*GameProperties.CUBE_SIZE, loc.getZ()*GameProperties.CUBE_SIZE);
    }
    ...
    public void setChunk(BlockChunkControl chunk)
    {
        ...
        System.out.println("ChunkLocation: " + chunk.getLocation() + " Blocks From: " + from + "  to: " + to);
        int xx = 0;
    ...
    {
        Block block = Blocks.SNOWBLOCK;
        if(on)
        {
            block = null;
        }
        on = !on;
        setBlock(new Vector3Int(2, 2, 2), block);
//        for(int x=0; x<2; x++)
//        {
//            for(int y=0; y< 2; y++)
//            {
