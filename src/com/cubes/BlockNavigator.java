...
public class BlockNavigator{
    ...
    public static Vector3Int getPointedBlockLocation(BTControl blockTerrain, Vector3f collisionContactPoint, boolean getNeighborLocation){
        ...
        Vector3Int blockLocation = new Vector3Int(
                (int) (collisionLocation.getX() / GameProperties.CUBE_SIZE),
                (int) (collisionLocation.getY() / GameProperties.CUBE_SIZE),
                (int) (collisionLocation.getZ() / GameProperties.CUBE_SIZE));
        if((blockTerrain.getPointedAtBlock(blockLocation) != null) == getNeighborLocation){
            if((collisionLocation.getX() % GameProperties.CUBE_SIZE) == 0) {
                blockLocation.subtractLocal(1, 0, 0);
            }
            else if((collisionLocation.getY() % GameProperties.CUBE_SIZE) == 0){
                blockLocation.subtractLocal(0, 1, 0);
            }
            else if((collisionLocation.getZ() % GameProperties.CUBE_SIZE) == 0){
