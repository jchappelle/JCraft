...
public class Perlin
{
    ...
    private char[] remap;
    private static String outputFile = "c:/jcraft/perlin.png";
    private static int seed = 123;

    public static void main(String[] args)
    {
        Perlin perlin = new Perlin(1024, 1024, 4, 2.5f, 140, 0.9875f, 6);

        try
        {
            ImageIO.write(perlin.image, "png", new File(outputFile));
        }
        catch (IOException e)
        {
            e.printStackTrace();
...
//    }
     ...
     *
     */
    public Perlin(int width, int height, int initFreq, float persistency, int density, float cloudSharpness, int detail)
    {
        ...
        {
            noiseMaps[i] = new Noise2D(width, height, initFreq, initFreq, amplitude);
            noiseMaps[i].start();
        ...
        adjustImage(Runtime.getRuntime().availableProcessors(), image.getWidth() * image.getHeight() / 0x40000);
    ...
    private static class Noise2D extends Thread
    {
        ...
        public void run()
        {
            BufferedImage temp = new BufferedImage(freqX, freqY, BufferedImage.TYPE_4BYTE_ABGR);

            Graphics2D g = temp.createGraphics();

            // generate a low-res random image

            Random random = new Random(seed); // use a consistent seed for now
